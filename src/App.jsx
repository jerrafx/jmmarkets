...
import { getAuth, signInAnonymously, onAuthStateChanged, signInWithCustomToken, signInWithEmailAndPassword } from 'firebase/auth';
      ...
      {textParts?.map((part, index) => ( 
        <span key={index} className={index === 0 ? themeColors.primaryAccent : themeColors.text}>
          {fullTypedText.startsWith(textParts.slice(0, index + 1).map(p=>p.text).join(''))
            ...
            : (index < currentPartIndex ? part.text : '') 
          }
        </span>
      ))}
...
const PageHeader = ({ title, onNavigateToDashboard, onNavigateToContact, onToggleTheme, currentTheme, themeColors, showHomeButton = false, onNavigateToHome }) => { 
  return ( <div className={`p-3 sm:p-4 ${themeColors.subtleBg} shadow-md flex justify-between items-center sticky top-0 z-40`}> <h1 className={`text-xl sm:text-2xl font-bold ${themeColors.primaryAccent}`}>{title}</h1> <div className="flex items-center space-x-1 sm:space-x-2"> {showHomeButton && onNavigateToHome && ( <button onClick={onNavigateToHome} title="Home" className={`p-2 rounded-md hover:${themeColors.inputBg} transition-colors ${themeColors.subtleText}`}> <Home size={18} sm:size={20} /> </button> )} {onNavigateToDashboard && ( <button onClick={onNavigateToDashboard} title="Dashboard Overzicht" className={`p-2 rounded-md hover:${themeColors.inputBg} transition-colors ${themeColors.subtleText}`}> <LayoutDashboard size={18} sm:size={20} /> </button> )} {onNavigateToContact && ( <button onClick={onNavigateToContact} title="Contact" className={`p-2 rounded-md hover:${themeColors.inputBg} transition-colors ${themeColors.subtleText}`}> <Send size={18} sm:size={20} /> </button> )} <button onClick={onToggleTheme} className={`p-2 rounded-md hover:${themeColors.inputBg} transition-colors ${themeColors.subtleText}`} title="Toggle Thema"> {currentTheme === 'dark' ? <Sun size={18} sm:size={20} /> : <Moon size={18} sm:size={20} />} </button> </div> </div> );
};
...
const MoodSmiley = ({ moodValue, themeColors, size = 18 }) => {
    ...
    if (!currentMood) {
        return <span className={`text-xs ${themeColors.subtleText}`}>Kies stemming</span>; // Of een placeholder icoon
    }
...
const AdminTradeInputForm = ({ themeColors, userId, accounts }) => { 
  ...
  const [mood, setMood] = useState(null); // Start met geen selectie
  ...
  const handleFileChange = async (event) => {
    ...
    if (file && userId) {
        ...
        const filePath = `artifacts/${appId}/users/${userId}/tradeImages/${Date.now()}_${file.name}`;
  ...
  const handleSubmit = async (e) => { 
    e.preventDefault(); 
    if (!userId) { setFeedback("Fout: Gebruiker niet ingelogd."); return; } 
    if (!selectedAccountId) { setFeedback("Selecteer een account."); return; } 
    if (!mood) { setFeedback("Selecteer een stemming voor de trade."); return; } // Stemming verplicht
    ...
    try { 
      ...
      setPair(''); setDate(new Date().toISOString().split('T')[0]); setTradeTime(new Date().toTimeString().slice(0,5)); setRiskAmount(''); setRr(''); setOutcome('win'); setSelectedAccountId(''); setImageUrl(''); setImageUrlOptional(''); setReasoning(''); setMood(null); setOptionalImageFile(null); // Reset mood naar null
      if(fileInputRef.current) fileInputRef.current.value = ""; 
      setTimeout(() => setFeedback(''), 3000); 
    } catch (error) { console.error("Fout bij toevoegen trade: ", error); setFeedback("Fout bij toevoegen trade. Zie console."); } 
  };
  const inputClass = `w-full p-3 rounded-md ${themeColors.inputBg} ${themeColors.inputText} ${themeColors.borderColor} border focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors`; 
  const labelClass = `block text-sm font-medium mb-1 ${themeColors.subtleText}`;
  ...
  return ( 
    <div className={`${themeColors.cardBg} p-6 rounded-xl shadow-lg`}> 
      ...
      <form onSubmit={handleSubmit} className="space-y-4"> 
        ...
        <div> <label className={labelClass}>Stemming (verplicht)</label> <div className="flex space-x-2 mt-1"> {[1, 2, 3, 4, 5].map(value => { const moodConfig = [ { icon: Frown, color: themeColors.mood[0], bgColor: themeColors.moodBg[0] }, { icon: Frown, color: themeColors.mood[1], bgColor: themeColors.moodBg[1] }, { icon: Meh, color: themeColors.mood[2], bgColor: themeColors.moodBg[2] }, { icon: Smile, color: themeColors.mood[3], bgColor: themeColors.moodBg[3] }, { icon: Smile, color: themeColors.mood[4], bgColor: themeColors.moodBg[4] }]; const Icon = moodConfig[value-1].icon; return ( <button key={value} type="button" onClick={() => setMood(value)} className={`p-2 rounded-full transition-all duration-150 ${mood === value ? `${moodConfig[value-1].bgColor} ring-2 ${moodConfig[value-1].color.replace('text-','ring-')}` : `${themeColors.inputBg} hover:opacity-70`}`}> <Icon className={`${moodConfig[value-1].color}`} size={24} /> </button> ); })} </div> </div> <button type="submit" disabled={uploading} className={`${themeColors.primaryAccentBg} hover:${themeColors.primaryAccentHoverBg} text-white font-semibold py-3 px-6 rounded-lg w-full text-lg transition-colors disabled:opacity-50`}> {uploading ? 'Bezig met uploaden...' : 'Trade Opslaan'} </button> {feedback && <p className={`mt-4 text-sm ${feedback.includes("succesvol") ? themeColors.primaryAccent : 'text-red-500'}`}>{feedback}</p>} </form> </div> );
};

const AdminAccountManagement = ({ themeColors, userId, onAccountAdded, accounts: initialAccounts, setAccounts: updateParentAccounts }) => { const [accountName, setAccountName] = useState(''); const [startKapitaal, setStartKapitaal] = useState(''); const [accountStatus, setAccountStatus] = useState('active'); const [feedback, setFeedback] = useState(''); const [showDeleteModal, setShowDeleteModal] = useState(false); const [accountToDelete, setAccountToDelete] = useState(null); const [localAccounts, setLocalAccounts] = useState(initialAccounts || []); useEffect(() => { setLocalAccounts(initialAccounts || []); }, [initialAccounts]); const handleSubmit = async (e) => { e.preventDefault(); if (!userId) { setFeedback("Fout: Gebruiker niet ingelogd."); return; } if (!accountName.trim()) { setFeedback("Account naam mag niet leeg zijn."); return; } const kapitaalNum = parseFloat(startKapitaal); if (isNaN(kapitaalNum) || kapitaalNum < 0) { setFeedback("Ongeldig startkapitaal ingevoerd."); return; } const newAccount = { userId, name: accountName, startKapitaal: kapitaalNum, status: accountStatus, createdAt: serverTimestamp() }; try { const accountsCollectionPath = `/artifacts/${appId}/users/${userId}/accounts`; const docRef = await addDoc(collection(db, accountsCollectionPath), newAccount); setFeedback(`Account "${accountName}" succesvol toegevoegd!`); const addedAccount = { id: docRef.id, ...newAccount }; if(onAccountAdded) onAccountAdded(addedAccount); setLocalAccounts(prev => [...prev, addedAccount]); setAccountName(''); setStartKapitaal(''); setAccountStatus('active'); setTimeout(() => setFeedback(''), 3000); } catch (error) { console.error("Fout bij toevoegen account: ", error); setFeedback("Fout bij toevoegen account. Zie console."); } }; const handleDeleteAttempt = (account) => { setAccountToDelete(account); setShowDeleteModal(true); }; const confirmDeleteAccount = async () => { if (!accountToDelete || !userId) return; try { const accountDocRef = doc(db, `/artifacts/${appId}/users/${userId}/accounts`, accountToDelete.id); await deleteDoc(accountDocRef); if (updateParentAccounts) { updateParentAccounts(prevAccounts => prevAccounts.filter(acc => acc.id !== accountToDelete.id)); } else { setLocalAccounts(prevAccounts => prevAccounts.filter(acc => acc.id !== accountToDelete.id)); } setFeedback(`Account "${accountToDelete.name}" succesvol verwijderd.`); setShowDeleteModal(false); setAccountToDelete(null); setTimeout(() => setFeedback(''), 3000); } catch (error) { console.error("Fout bij verwijderen account:", error); setFeedback("Fout bij verwijderen account."); setShowDeleteModal(false); setAccountToDelete(null); } }; const inputClass = `w-full p-3 rounded-md ${themeColors.inputBg} ${themeColors.inputText} ${themeColors.borderColor} border focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors`; const labelClass = `block text-sm font-medium mb-1 ${themeColors.subtleText}`; return ( <> <div className={`${themeColors.cardBg} p-6 rounded-xl shadow-lg mt-8`}> <h3 className={`text-xl font-semibold mb-6 ${themeColors.text === 'text-slate-200' ? 'text-white' : 'text-slate-900'}`}>Account Toevoegen/Beheren</h3> <form onSubmit={handleSubmit} className="space-y-4"> <div><label htmlFor="accountName" className={labelClass}>Account Naam</label><input type="text" id="accountName" value={accountName} onChange={e => setAccountName(e.target.value)} className={inputClass} placeholder="bijv. Persoonlijk EUR" required /></div> <div><label htmlFor="startKapitaal" className={labelClass}>Startkapitaal in $</label><input type="number" step="0.01" id="startKapitaal" value={startKapitaal} onChange={e => setStartKapitaal(e.target.value)} className={inputClass} placeholder="bijv. 10000" required /></div> <div> <label htmlFor="accountStatus" className={labelClass}>Status</label> <select id="accountStatus" value={accountStatus} onChange={e => setAccountStatus(e.target.value)} className={inputClass}> <option key="status-active" value="active">Actief</option> <option key="status-inactive" value="inactive">Inactief</option> </select> </div> <button type="submit" className={`${themeColors.primaryAccentBg} hover:${themeColors.primaryAccentHoverBg} text-white font-semibold py-3 px-6 rounded-lg w-full text-lg transition-colors`}>Account Opslaan</button> {feedback && <p className={`mt-4 text-sm ${feedback.includes("succesvol") ? themeColors.primaryAccent : 'text-red-500'}`}>{feedback}</p>} </form> </div> {localAccounts && localAccounts.length > 0 && ( <div className={`${themeColors.cardBg} p-6 rounded-xl shadow-lg mt-8`}> <h4 className={`text-lg font-semibold mb-4 ${themeColors.text === 'text-slate-200' ? 'text-white' : 'text-slate-900'}`}>Bestaande Accounts</h4> <ul className="space-y-2"> {localAccounts.map((acc, index) => ( <li key={`${acc.id}-${index}`} className={`flex justify-between items-center p-3 rounded-md ${themeColors.subtleBg}`}> <div> <p className={themeColors.cardText}>{acc.name}</p> <p className={`text-xs ${themeColors.subtleText}`}>Start: ${(acc.startKapitaal || 0).toLocaleString('nl-NL', {minimumFractionDigits: 2, maximumFractionDigits: 2})} | Aangemaakt: {acc.createdAt && typeof acc.createdAt.toDate === 'function' ? acc.createdAt.toDate().toLocaleDateString('nl-NL') : 'N/A'}</p> </div> <div className="flex items-center space-x-2"> <span className={`text-xs px-2 py-0.5 rounded-full ${acc.status === 'active' ? 'bg-green-500/30 text-green-300' : 'bg-red-500/30 text-red-300'}`}>{acc.status}</span> <button onClick={() => handleDeleteAttempt(acc)} className="p-1 text-red-500 hover:text-red-400 transition-colors"> <Trash2 size={18}/> </button> </div> </li> ))} </ul> </div> )} {showDeleteModal && accountToDelete && ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"> <div className={`${themeColors.cardBg} p-6 rounded-lg shadow-xl max-w-sm w-full`}> <div className="flex items-center mb-4"> <AlertTriangle size={24} className="text-red-500 mr-3"/> <h4 className={`text-lg font-semibold ${themeColors.text}`}>Account Verwijderen</h4> </div> <p className={`${themeColors.cardText} text-sm mb-6`}> Weet u zeker dat u het account "{accountToDelete.name}" wilt verwijderen? Alle bijbehorende trades blijven bestaan maar zijn niet meer gekoppeld. Deze actie kan niet ongedaan worden gemaakt. </p> <div className="flex justify-end space-x-3"> <button onClick={() => { setShowDeleteModal(false); setAccountToDelete(null); }} className={`px-4 py-2 text-sm rounded-md ${themeColors.inputBg} ${themeColors.inputText} hover:opacity-80 transition-opacity`} > Annuleren </button> <button onClick={confirmDeleteAccount} className="px-4 py-2 text-sm rounded-md bg-red-600 hover:bg-red-700 text-white transition-colors" > Verwijder </button> </div> </div> </div> )} </> );};
const UserAccountStatusList = ({ themeColors, userId, allTrades, onSelectAccountTrades }) => { const [accounts, setAccounts] = useState([]); const [loading, setLoading] = useState(true); const [expandedAccountId, setExpandedAccountId] = useState(null); useEffect(() => { if (!userId) { setLoading(false); return; } const accountsCollectionPath = `/artifacts/${appId}/users/${userId}/accounts`; const q = query(collection(db, accountsCollectionPath)); const unsubscribe = onSnapshot(q, (querySnapshot) => { const accs = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })); setAccounts(accs); setLoading(false); }, (error) => { console.error("Fout bij ophalen accounts: ", error); setLoading(false); }); return () => unsubscribe(); }, [userId]); const toggleDetails = (accountId) => { setExpandedAccountId(prevId => (prevId === accountId ? null : accountId)); }; const getAccountDetails = (account) => { const tradesForAccount = allTrades.filter(trade => trade.accountId === account.id); const totalPnlForAccount = tradesForAccount.reduce((sum, trade) => sum + trade.pnl, 0); const currentBalance = (account.startKapitaal || 0) + totalPnlForAccount; const pnlSinceStart = currentBalance - (account.startKapitaal || 0); return { tradeCount: tradesForAccount.length, totalPnl: totalPnlForAccount, currentBalance: currentBalance, pnlSinceStart: pnlSinceStart }; }; if (loading) return <p className={`${themeColors.subtleText} text-center py-4`}>Account status laden...</p>; if (accounts.length === 0) return <p className={`${themeColors.subtleText} text-center py-4`}>Nog geen accounts beschikbaar.</p>; return ( <div className={`${themeColors.cardBg} p-4 sm:p-6 rounded-xl shadow-lg mt-8`}> <h3 className={`text-lg sm:text-xl font-semibold mb-4 ${themeColors.text === 'text-slate-200' ? 'text-white' : 'text-slate-900'}`}>Account Status <em className={`text-xs ${themeColors.subtleText} opacity-75`}>Hier kan u ook bij staan!</em></h3> <ul className="space-y-3"> {accounts.map((acc, index) => { if (!acc || typeof acc.id !== 'string' || acc.id.trim() === '') { console.warn("UserAccountStatusList: Ongeldig account of account ID", acc); return null;} const details = getAccountDetails(acc); return ( <li key={`${acc.id}-${index}`} className={`${themeColors.subtleBg} rounded-md overflow-hidden`}> <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 `}> <div className="flex-grow mb-2 sm:mb-0"> <span className={`${themeColors.cardText} font-semibold`}>{acc.name}</span> <div className="flex flex-col sm:flex-row items-start sm:items-center text-xs mt-1"> <span className={`${themeColors.subtleText} mr-2 mb-1 sm:mb-0`}>Huidig: {details.currentBalance.toLocaleString('nl-NL', { style: 'currency', currency: 'USD' })}</span> <span className={`${details.pnlSinceStart >= 0 ? themeColors.textPositive : themeColors.textNegative}`}> {details.pnlSinceStart >= 0 ? '+' : ''}{details.pnlSinceStart.toLocaleString('nl-NL', { style: 'currency', currency: 'USD' })} </span> </div> </div> <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0 self-start sm:self-center"> <span className={`text-xs ${themeColors.subtleText} mr-2`}>Trades: {details.tradeCount}</span> <button onClick={() => onSelectAccountTrades(acc.id)} className={`px-2 py-1 text-xs rounded-md ${themeColors.filterButtonInactiveBg} ${themeColors.filterButtonInactiveText} hover:opacity-80 transition-colors`} > Bekijk </button> <button onClick={() => toggleDetails(acc.id)} className={`${themeColors.primaryAccent} hover:opacity-75 p-1 rounded`}> {expandedAccountId === acc.id ? <ChevronUp size={16} sm:size={18}/> : <ChevronDown size={16} sm:size={18}/>} </button> </div> </div> {expandedAccountId === acc.id && ( <div className={`p-3 border-t ${themeColors.borderColor} bg-opacity-50 ${themeColors.inputBg} text-xs sm:text-sm`}> <p className={`${themeColors.cardText}`}>Startkapitaal: <span className="font-semibold">{(acc.startKapitaal || 0).toLocaleString('nl-NL', { style: 'currency', currency: 'USD' })}</span></p> <p className={`${themeColors.cardText}`}>P/L van Trades: <span className={`font-semibold ${details.totalPnl >= 0 ? themeColors.textPositive : themeColors.textNegative}`}>{details.totalPnl.toLocaleString('nl-NL', { style: 'currency', currency: 'USD' })}</span></p> </div> )} </li> ); })} </ul> </div> );};
const KPI_Card = ({ title, value, icon, unit = '', themeColors, isPnl = false }) => { const IconComponent = icon; let valueColor = themeColors.text === 'text-slate-200' ? 'text-white' : 'text-slate-900'; if (isPnl) { const numericString = String(value).replace(/[^\d,.-]/g, '').replace(',', '.'); const numericValue = parseFloat(numericString); if (!isNaN(numericValue)) { if (numericValue > 0) valueColor = themeColors.textPositive; else if (numericValue < 0) valueColor = themeColors.textNegative; } } return ( <div className={`${themeColors.cardBg} p-4 sm:p-5 md:p-6 rounded-xl shadow-lg flex items-center space-x-3 sm:space-x-4 transition-all duration-300 hover:shadow-blue-500/20`}> <div className={`p-2 sm:p-3 rounded-full ${themeColors.primaryAccentBg.replace('bg-','bg-opacity-20')} ${themeColors.primaryAccent}`}> {IconComponent && <IconComponent size={20} sm:size={24} />} </div> <div> <p className={`text-xs sm:text-sm ${themeColors.subtleText}`}>{title}</p> <p className={`text-xl sm:text-2xl font-bold ${valueColor}`}>{value}{unit}</p> </div> </div> );};
const SimpleEquityChart = ({ data, themeColors }) => { if (!data || data.length === 0) { return <p className={`${themeColors.subtleText} text-center py-10`}>Onvoldoende data voor grafiek.</p>; } if (data.length === 1 && data[0].date === "Start") { return <p className={`${themeColors.subtleText} text-center py-10`}>Voeg trades toe om de equity curve te zien.</p>; } const chartHeight = 380; const chartWidth = 750; const yPadding = 70; const xPadding = 90; const equities = data.map(p => p.equity); let minEquity = Math.min(...equities); let maxEquity = Math.max(...equities); const dataRange = maxEquity - minEquity; const visualBuffer = dataRange === 0 ? 100 : dataRange * 0.20; let visualMinEquity = minEquity - visualBuffer; let visualMaxEquity = maxEquity + visualBuffer; if (minEquity >= 0 && visualMinEquity < 0) { visualMinEquity = 0; } if (visualMaxEquity - visualMinEquity < 250 && dataRange === 0) { visualMinEquity = equities[0] - 125; visualMaxEquity = equities[0] + 125; } else if (visualMaxEquity - visualMinEquity < 250) { const mid = (visualMaxEquity + visualMinEquity) / 2; visualMinEquity = mid - 125; visualMaxEquity = mid + 125; } visualMinEquity = Math.floor(visualMinEquity / 50) * 50; visualMaxEquity = Math.ceil(visualMaxEquity / 50) * 50; const visualRange = visualMaxEquity - visualMinEquity === 0 ? 250 : visualMaxEquity - visualMinEquity; const linePath = (points) => { if (points.length === 0) return ""; let path = `M ${points[0].x} ${points[0].y}`; for (let i = 0; i < points.length - 1; i++) { const x_mid = (points[i].x + points[i + 1].x) / 2; const cp_x1 = (x_mid + points[i].x) / 2; const cp_x2 = (x_mid + points[i + 1].x) / 2; path += ` C ${cp_x1},${points[i].y} ${cp_x2},${points[i+1].y} ${points[i+1].x},${points[i+1].y}`; } return path; }; const chartPoints = data.map((point, index) => { const x = (index / Math.max(1, data.length - 1)) * (chartWidth - 2 * xPadding) + xPadding; const y = chartHeight - yPadding - ((point.equity - visualMinEquity) / visualRange) * (chartHeight - 2 * yPadding); return {x, y}; }); const areaPath = `M ${xPadding},${chartHeight - yPadding} L ${chartPoints.map(p => `${p.x},${p.y}`).join(' L ')} L ${chartWidth - xPadding},${chartHeight - yPadding} Z`; const yAxisLabels = []; const numYLabels = 5; const yStep = Math.max(250, Math.ceil(visualRange / (numYLabels -1) / 250) * 250); let currentYLabel = Math.floor(visualMinEquity / yStep) * yStep; if (currentYLabel < visualMinEquity && visualMinEquity !== 0) currentYLabel += yStep; if (visualMinEquity === 0 && currentYLabel < 0) currentYLabel = 0; for (let val = currentYLabel; val <= visualMaxEquity + yStep/2 ; val += yStep) { if (val >= visualMinEquity - yStep/2) { const yPos = chartHeight - yPadding - ((val - visualMinEquity) / visualRange) * (chartHeight - 2 * yPadding); yAxisLabels.push({ value: val.toLocaleString('nl-NL', { style: 'decimal', minimumFractionDigits:0, maximumFractionDigits:0 }), y: yPos }); } } if (yAxisLabels.length === 0 && visualRange > 0) { yAxisLabels.push({ value: visualMinEquity.toLocaleString('nl-NL', { style: 'decimal', minimumFractionDigits:0, maximumFractionDigits:0 }), y: chartHeight - yPadding }); yAxisLabels.push({ value: visualMaxEquity.toLocaleString('nl-NL', { style: 'decimal', minimumFractionDigits:0, maximumFractionDigits:0 }), y: yPadding }); } const xAxisLabels = []; if (data.length > 0) { const firstPointDate = data[0].date === "Start" && data.length > 1 ? data[1].date : data[0].date; xAxisLabels.push({ text: firstPointDate, x: xPadding }); if (data.length > 2) { const midIndex = Math.floor(data.length / 2); if (data[midIndex].date !== firstPointDate && data[midIndex].date !== data[data.length -1].date) { xAxisLabels.push({ text: data[midIndex].date, x: chartWidth / 2 }); } } if (data.length > 1 && data[data.length - 1].date !== firstPointDate) { xAxisLabels.push({ text: data[data.length - 1].date, x: chartWidth - xPadding }); } } return ( <div className="w-full overflow-x-auto flex justify-center"> <svg width={chartWidth} height={chartHeight + 60} viewBox={`0 0 ${chartWidth} ${chartHeight + 60}`} className="max-w-full font-inter"> <defs> <linearGradient id="equityGradient" x1="0%" y1="0%" x2="0%" y2="100%"> <stop offset="0%" stopColor={themeColors.chartGradientFrom} /> <stop offset="100%" stopColor={themeColors.chartGradientTo} /> </linearGradient> </defs> <text x={xPadding / 3 - 15} y={chartHeight / 2} transform={`rotate(-90 ${xPadding/3 - 15} ${chartHeight/2})`} textAnchor="middle" className={`text-sm font-semibold ${themeColors.chartLabel}`}> Equity ($) </text> <text x={chartWidth - xPadding + 30} y={chartHeight + yPadding } textAnchor="end" className={`text-sm font-semibold ${themeColors.chartLabel}`}> Datum </text> {yAxisLabels.map((label, i) => ( <g key={`y-axis-group-${i}`}> <line x1={xPadding} y1={label.y} x2={chartWidth - xPadding} y2={label.y} className={themeColors.chartGrid} strokeOpacity="0.3" strokeDasharray="4 4" /> <text x={xPadding - 15} y={label.y + 4} textAnchor="end" className={`text-xs ${themeColors.chartAxis}`}>${label.value}</text> </g> ))} {xAxisLabels.map((label, i) => ( <text key={`x-label-${i}`} x={label.x} y={chartHeight - yPadding + 35} textAnchor={i === 0 ? "start" : i === xAxisLabels.length -1 && xAxisLabels.length > 1 ? "end" : "middle"} className={`text-xs ${themeColors.chartAxis}`}>{label.text}</text> ))} {chartPoints.length > 1 && ( <> <path d={areaPath} fill="url(#equityGradient)" opacity="0.6" /> <path d={linePath(chartPoints)} fill="none" className={themeColors.chartLine} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /> <circle cx={chartPoints[chartPoints.length-1].x} cy={chartPoints[chartPoints.length-1].y} r="4.5" className={themeColors.chartPoint} stroke={themeColors.cardBg} strokeWidth="2.5"/> </> )} </svg> </div> );};
const DashboardNavbar = ({ onToggleTheme, currentTheme, themeColors, onNavigateToTimeline, onNavigateToContact, isAdmin, onAdminLogout, onTabChange, activeTab }) => { const baseNavLinks = [ { name: 'Overzicht', icon: LayoutDashboard, action: () => onTabChange('overview') }, { name: 'Mijn Reis', icon: User, action: onNavigateToTimeline }, { name: 'Contact', icon: Send, action: onNavigateToContact }, ]; if (isAdmin) { baseNavLinks.push({ name: 'Admin', icon: ShieldCheck, action: () => onTabChange('admin') }); } return ( <nav className={`${themeColors.cardBg} ${themeColors.text} p-3 sm:p-4 shadow-lg flex justify-between items-center sticky top-0 z-50 font-inter`}> <div className="flex items-center"> <span className={`text-lg sm:text-xl font-bold ${themeColors.primaryAccent}`}>J<span className={themeColors.text}>Markets.nl</span></span> </div> <div className="flex items-center space-x-1"> {baseNavLinks.map(link => { const Icon = link.icon; return ( <button key={link.name} onClick={() => { if(link.action) link.action(); }} title={link.name} className={`p-2 rounded-md hover:${themeColors.inputBg} transition-colors flex items-center text-xs sm:text-sm ${activeTab === link.name.toLowerCase() ? `${themeColors.primaryAccentBg} text-white` : `${themeColors.subtleText}`}`}> <Icon size={18} sm:size={20}/> <span className="hidden sm:inline ml-1 sm:ml-2">{link.name}</span> </button> )})} {isAdmin && ( <button onClick={onAdminLogout} title="Admin Uitloggen" className={`p-2 rounded-md hover:bg-red-700 bg-red-600 text-white transition-colors flex items-center text-xs sm:text-sm`}> <LogOut size={18} sm:size={20} /> <span className="hidden sm:inline ml-1 sm:ml-2">Logout</span> </button> )} <button onClick={onToggleTheme} className={`p-2 rounded-md hover:${themeColors.inputBg} transition-colors ${themeColors.subtleText}`} title="Toggle Thema"> {currentTheme === 'dark' ? <Sun size={18} sm:size={20} /> : <Moon size={18} sm:size={20} />} </button> </div> </nav> );};
...
const DashboardView = ({ onBackToLanding, currentTheme, toggleTheme, themeColors, onNavigateToTimeline, onNavigateToContact, userId, isAdmin, onAdminLogout }) => { 
    ...
    useEffect(() => { if (userId) { const accountsPath = `/artifacts/${appId}/users/${userId}/accounts`; const unsubAccounts = onSnapshot(query(collection(db, accountsPath)), (snap) => { const fetchedAccounts = snap.docs.map(d => ({ id: d.id, ...d.data() })); setAccounts(fetchedAccounts); }); return () => unsubAccounts(); } else { setAccounts([]); } }, [userId]); 
    
    useEffect(() => { 
        if (userId && (selectedAccountFilterId === 'cumulative' || accounts.length > 0)) { 
            setTradesLoading(true); 
            const tradesPath = `/artifacts/${appId}/users/${userId}/trades`; 
            ...
            const unsubTrades = onSnapshot(tradesQuery, (snapshot) => { 
                ...
                tradesToProcess.forEach((trade, index) => { 
                    ...
                    if (trade.outcome === 'win') { 
                        ...
                        losingTrades++; 
                        sumOfPnlLosses += Math.abs(trade.pnl); 
            ...
            }, (error) => { console.error("Fout bij ophalen trades: ", error); setTradesLoading(false); }); 
            return () => unsubTrades(); 
        } else if (!userId) { 
            setAccounts([]); 
            setEquityCurveData([]); 
    ...
    }, [userId, accounts, selectedAccountFilterId]); 
    
    useEffect(() => { if (!isAdmin && activeMainTab === 'admin') { setActiveMainTab('overview'); } }, [isAdmin, activeMainTab]); 
    
    const handleAccountAdded = (newAccount) => { setAccounts(prev => [...prev, newAccount]); }; 
    
    const kpiData = [ { title: "Totaal Netto Winst/Verlies", value: kpiValues.totalPL.toLocaleString('nl-NL', { style: 'currency', currency: 'USD' }), icon: DollarSign, isPnl: true }, { title: "Win Rate", value: kpiValues.winRate.toFixed(1), icon: Percent, unit: '%' }, { title: "Gem. Risk/Reward Ratio", value: kpiValues.avgRRR.toFixed(2), icon: TrendingUp }, { title: "Totaal Aantal Trades", value: kpiValues.totalTrades, icon: BarChart2 }, ]; 
...
const App = () => { const [currentView, setCurrentView] = useState('landing'); const [currentUser, setCurrentUser] = useState(null); const [isAuthReady, setIsAuthReady] = useState(false); const [theme, setTheme] = useState('light'); const [isAdmin, setIsAdmin] = useState(false); const currentThemeColors = themes[theme]; const navigateTo = (view) => { const pageContainer = document.getElementById('page-container'); if (pageContainer) { pageContainer.style.opacity = '0'; setTimeout(() => { setCurrentView(view); window.scrollTo(0, 0); const innerContainer = document.querySelector('.journey-container, .flex-grow.overflow-y-auto'); if(innerContainer) innerContainer.scrollTop = 0; setTimeout(() => { pageContainer.style.opacity = '1'; }, 50); }, 250); } else { setCurrentView(view); window.scrollTo(0,0); } }; useEffect(() => { const authListener = onAuthStateChanged(auth, async (user) => { if (user) { setCurrentUser(user); const userDocRef = doc(db, `/artifacts/${appId}/users/${user.uid}/profile`, 'info'); const userDocSnap = await getDoc(userDocRef); if (!userDocSnap.exists()) { try { await setDoc(userDocRef, { email: user.email || 'anoniem', name: user.displayName || 'Jeremy Mlynarczyk', createdAt: Timestamp.now(), themePreference: 'light', isAdmin: false }); setTheme('light'); setIsAdmin(false); } catch (error) { console.error("Fout bij aanmaken gebruikersprofiel:", error); } } else { const profileData = userDocSnap.data(); setTheme(profileData?.themePreference || 'light'); setIsAdmin(profileData?.isAdmin || false); } } else { if (typeof __initial_auth_token === 'undefined' || !__initial_auth_token) { signInAnonymously(auth).catch(error => console.error("Anoniem inloggen mislukt:", error)); } setCurrentUser(null); setIsAdmin(false); } setIsAuthReady(true); }); const attemptCustomSignIn = async () => { if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) { try { await signInWithCustomToken(auth, __initial_auth_token); } catch (error) { console.error("Custom token inloggen mislukt, probeer anoniem:", error); signInAnonymously(auth).catch(anonError => console.error("Anoniem inloggen ook mislukt:", anonError)); } } }; if (!auth.currentUser) { attemptCustomSignIn(); } return () => authListener(); }, []); const toggleTheme = async () => { const newTheme = theme === 'dark' ? 'light' : 'dark'; setTheme(newTheme); if (currentUser) { try { const userDocRef = doc(db, `/artifacts/${appId}/users/${currentUser.uid}/profile`, 'info'); await updateDoc(userDocRef, { themePreference: newTheme }); } catch (error) { console.error("Fout bij updaten thema voorkeur:", error); } } }; const handleSuccessfulAdminLogin = async () => { if (currentUser) { const userDocRef = doc(db, `/artifacts/${appId}/users/${currentUser.uid}/profile`, 'info'); try { const userDocSnap = await getDoc(userDocRef); if (userDocSnap.exists()) { await updateDoc(userDocRef, { isAdmin: true }); } else { await setDoc(userDocRef, { email: currentUser.email || 'anoniem', name: currentUser.displayName || 'Jeremy Mlynarczyk', createdAt: Timestamp.now(), themePreference: theme, isAdmin: true }); } setIsAdmin(true); navigateTo('dashboard'); console.log("Admin status geactiveerd voor user:", currentUser.uid); } catch (error) { console.error("Fout bij instellen admin status:", error); alert("Kon admin status niet instellen. Probeer opnieuw."); } } else { alert("Geen gebruiker ingelogd. Kan admin status niet instellen. Probeer de pagina te verversen."); } }; const handleAdminLogout = async () => { if (currentUser) { const userDocRef = doc(db, `/artifacts/${appId}/users/${currentUser.uid}/profile`, 'info'); try { await updateDoc(userDocRef, { isAdmin: false }); setIsAdmin(false); navigateTo('dashboard'); console.log("Admin status gedeactiveerd."); } catch (error) { console.error("Fout bij deactiveren admin status:", error); } } }; const pageProps = { theme, toggleTheme, themeColors: currentThemeColors, isAdmin }; let viewComponent; if (currentView === 'landing') { viewComponent = <LandingPage onNavigateToDashboard={() => navigateTo('dashboard')} onNavigateToTimeline={() => navigateTo('timeline')} onNavigateToContact={() => navigateTo('contact')} onNavigateToAdminLogin={() => navigateTo('adminLogin')} {...pageProps} />; } else if (currentView === 'adminLogin') { viewComponent = <AdminLoginPage onSuccessfulAdminLogin={handleSuccessfulAdminLogin} onBackToLanding={() => navigateTo('landing')} themeColors={currentThemeColors} />; } else if (currentView === 'timeline') { viewComponent = <MyJourneyTimelinePage onBackToHome={() => navigateTo('landing')} onNavigateToDashboard={() => navigateTo('dashboard')} onNavigateToContact={() => navigateTo('contact')} {...pageProps} />; } else if (currentView === 'contact') { viewComponent = <ContactPage onBackToHome={() => navigateTo('landing')} onNavigateToDashboard={() => navigateTo('dashboard')} {...pageProps} />; } else if (currentView === 'dashboard') { viewComponent = <DashboardView onBackToLanding={() => navigateTo('landing')} currentTheme={theme} toggleTheme={toggleTheme} themeColors={currentThemeColors} onNavigateToTimeline={() => navigateTo('timeline')} onNavigateToContact={() => navigateTo('contact')} userId={currentUser?.uid} isAdmin={isAdmin} onAdminLogout={handleAdminLogout}/>; } else { viewComponent = <div className={`min-h-screen ${currentThemeColors.bg} flex justify-center items-center ${currentThemeColors.text} font-inter`}><p>Ongeldige weergave.</p></div>; } if (!isAuthReady) { return <div className={`min-h-screen ${currentThemeColors.bg} flex justify-center items-center ${currentThemeColors.text} font-inter`}><p>Authenticatie laden...</p></div>; } return ( <div className="flex flex-col min-h-screen"> <div id="page-container" className="flex-grow transition-opacity duration-200 ease-in-out" style={{opacity: 1}}> {viewComponent} </div> <FloatingCallButton themeColors={currentThemeColors} /> <Footer themeColors={currentThemeColors} /> </div> );};
// export { PageHeader, TimelineEvent }; // Verwijderd om de build error te voorkomen
export default App;
