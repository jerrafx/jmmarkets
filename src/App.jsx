...
const DashboardView = ({ onBackToLanding, currentTheme, toggleTheme, themeColors, onNavigateToTimeline, onNavigateToContact, userId, isAdmin, onAdminLogout, navigateTo }) => { 
    ...
    useEffect(() => { 
        const dataOwnerId = isAdmin ? ADMIN_DATA_OWNER_ID : userId;
        if (dataOwnerId) { 
    ...
    useEffect(() => { 
        ...
        if (dataOwnerId && (selectedAccountFilterId === 'cumulative' || accounts.length > 0 || isAdmin)) { 
    ...
    }, [userId, accounts, selectedAccountFilterId, isAdmin]); 
...
const App = () => { const [currentView, setCurrentView] = useState('landing'); const [currentUser, setCurrentUser] = useState(null); const [isAuthReady, setIsAuthReady] = useState(false); const [theme, setTheme] = useState('light'); const [isAdmin, setIsAdmin] = useState(false); const currentThemeColors = themes[theme]; const navigateTo = (view) => { const pageContainer = document.getElementById('page-container'); if (pageContainer) { pageContainer.style.opacity = '0'; setTimeout(() => { setCurrentView(view); window.scrollTo(0, 0); const innerContainer = document.querySelector('.journey-container, .flex-grow.overflow-y-auto'); if(innerContainer) innerContainer.scrollTop = 0; setTimeout(() => { pageContainer.style.opacity = '1'; }, 50); }, 250); } else { setCurrentView(view); window.scrollTo(0,0); } }; useEffect(() => { const authListener = onAuthStateChanged(auth, async (user) => { console.log("Auth state changed, Firebase user:", user); if (user) { setCurrentUser(user); 
        ...
        if (userDocSnap.exists()) {
            ...
            // Als de ingelogde Firebase user THE_ADMIN_UID is EN zijn profiel isAdmin:true heeft,
            // EN de lokale isAdmin state nog niet true is (om PIN login niet te overschrijven),
            // dan zetten we de lokale isAdmin state. Dit is een fallback.
            if (user.uid === THE_ADMIN_UID && profileData?.isAdmin && !isAdmin) { 
                setIsAdmin(true);
                console.log("Admin (", user.uid, ") herkend via Firebase Auth & Firestore profiel.");
            }
        } else if (user.uid !== ADMIN_DATA_OWNER_ID && user.uid !== THE_ADMIN_UID) { 
             try { 
    ...
    setIsAuthReady(true); 
}); const attemptCustomSignIn = async () => { if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) { try { console.log("Probeert custom token sign-in..."); await signInWithCustomToken(auth, __initial_auth_token); console.log("Custom token sign-in succesvol."); } catch (error) { console.error("Custom token inloggen mislukt, probeer anoniem:", error); signInAnonymously(auth).catch(anonError => console.error("Anoniem inloggen ook mislukt na custom token fail:", anonError)); } } }; if (!auth.currentUser && (typeof __initial_auth_token !== 'undefined' && __initial_auth_token)) { attemptCustomSignIn(); } else if (!auth.currentUser && (typeof __initial_auth_token === 'undefined' || !__initial_auth_token)) { signInAnonymously(auth).catch(error => console.error("Initiële anonieme login mislukt:", error)); } return () => authListener(); }, [isAdmin]); 
const toggleTheme = async () => { const newTheme = theme === 'dark' ? 'light' : 'dark'; setTheme(newTheme); 
    const uidToUpdate = isAdmin && currentUser?.isPseudoAdmin ? ADMIN_DATA_OWNER_ID : currentUser?.uid;

    if (uidToUpdate && uidToUpdate !== ADMIN_DATA_OWNER_ID) { 
        try { 
    ...
    } else if (isAdmin && uidToUpdate === ADMIN_DATA_OWNER_ID) { 
         try { 
            const userDocRef = doc(db, `/artifacts/${appId}/users/${ADMIN_DATA_OWNER_ID}/profile`, 'info'); 
            const userDocSnap = await getDoc(userDocRef);
            if(userDocSnap.exists()){
                 await updateDoc(userDocRef, { themePreference: newTheme }); 
            } else { 
                 await setDoc(userDocRef, { themePreference: newTheme, name: "Admin Data Storage" }, {merge: true}); 
            }
            console.log("Thema voorkeur geüpdatet in Firestore voor ADMIN_DATA_OWNER_ID");
        } catch (error) { 
            console.error("Fout bij updaten thema voorkeur voor admin data eigenaar:", error); 
        } 
    }
}; 
...
const handleAdminLogout = async () => { 
    ...
    if (auth.currentUser && !auth.currentUser.isAnonymous) {
        try {
            ...
            console.log("Firebase signOut succesvol voor niet-anonieme gebruiker."); 
            setCurrentUser(null); 
        } catch (error) {
...
return ( <div className="flex flex-col min-h-screen"> <div id="page-container" className="flex-grow transition-opacity duration-200 ease-in-out" style={{opacity: 1}}> {viewComponent} </div> <FloatingCallButton themeColors={currentThemeColors} /> <Footer themeColors={currentThemeColors} /> </div> );
};
export default App;
